...
def missing_labels_dice_metric(y_true, y_pred, eps=tf.keras.backend.epsilon(), reduce_along_batch=False, reduce_along_features=False, feature_weights=None, calibrate = False, **kwargs):
    missing_labels = tf.cast(tf.reduce_sum(y_true[:,:,:,:,1:],axis=(1,2,3),keepdims = True)==0,tf.float32)
    background_average = (y_pred[:,:,:,:,0:1] + tf.reduce_sum(missing_labels * y_pred[:,:,:,:,1:],axis = 4,keepdims = True)) / (tf.reduce_sum(missing_labels,axis = 4,keepdims = True)+1)
    background_expanded = tf.repeat(background_average,y_pred.shape[4]-1,axis = 4)
    y_pred_adapted = missing_labels * background_expanded + (1-missing_labels) * y_pred[:,:,:,:,1:]
    y_pred_adapted = tf.concat([background_average, y_pred_adapted], axis=4)

    background_true_average = y_true[:, :, :, :, 0:1] / (tf.reduce_sum(missing_labels, axis=4, keepdims=True) + 1)
    background_true_expanded = tf.repeat(background_true_average, y_pred.shape[4] - 1, axis=4)
    y_true_adapted = missing_labels * background_true_expanded + (1 - missing_labels) * y_true[:, :, :, :, 1:]
    y_true_adapted = tf.concat([background_true_average, y_true_adapted], axis=4)

    DSC = generalized_dice_coeff(y_true_adapted, y_pred_adapted, keepdims=True, eps=eps, reduce_along_batch=reduce_along_batch,
                                  reduce_along_features=reduce_along_features, feature_weights=None) # feature_weights has only effect if reduce_along_features = True

    if feature_weights is not None:
        DSC = DSC*feature_weights

    if calibrate:
        DSC = (DSC.shape[4] / (tf.reduce_sum(1 - missing_labels, axis=4, keepdims=True)+1)) * DSC


    return DSC

def leaf_dice_metric(y_true, y_pred, eps=tf.keras.backend.epsilon(), reduce_along_batch=False, reduce_along_features=False, feature_weights=None, **kwargs):
    missing_labels = tf.cast(tf.reduce_sum(y_true[:,:,:,:,1:],axis=(1,2,3),keepdims = True)==0,tf.float32)
...
def marginal_dice_metric(y_true, y_pred, eps=tf.keras.backend.epsilon(), reduce_along_batch=False, reduce_along_features=False, feature_weights=None, **kwargs):
    missing_labels = tf.cast(tf.reduce_sum(y_true[:,:,:,:,1:],axis=(1,2,3),keepdims = True)==0,tf.float32)
    ...
    DSC =  generalized_dice_coeff(y_true, y_pred_adapted, keepdims=True, eps=eps, reduce_along_batch=reduce_along_batch,
                                  reduce_along_features=reduce_along_features, feature_weights=feature_weights)
...
def get_metric(
    ...
    if metric_name == "missing_labels_dice_metric":
        metric = missing_labels_dice_metric

    elif metric_name == "leaf_dice_metric":
